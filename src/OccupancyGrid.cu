...
OccupancyGrid::OccupancyGrid(const int nr_voxels_per_dim, const float grid_extent, const torch::Tensor &grid_translation):
    m_nr_voxels_per_dim(nr_voxels_per_dim),
    m_grid_extent(grid_extent),
    m_grid_translation_tensor(grid_translation)
    {
...
void OccupancyGrid::update_with_sdf_positions(const torch::Tensor &points, const torch::Tensor &sdf, const float inv_s, const float occupancy_thresh)
{
    CHECK(sdf.dim()==2) << "density should have dim 2 correspondin to nr_pointsx1. However it has sizes" << sdf.sizes();
    // CHECK(decay<1.0) <<"We except the decay to be <1.0 but it is " << decay;
    CHECK(points.dim()==2) << "point_indices should have dim 2 correspondin to nr_pointsx3. However it has sizes" << points.sizes();

    int nr_points=points.size(0);
    const dim3 blocks = { (unsigned int)div_round_up(nr_points, BLOCK_SIZE), 1, 1 }; 

    OccupancyGridGPU::update_with_sdf_positions_gpu<<<blocks, BLOCK_SIZE>>>(
                nr_points,
                sdf.packed_accessor32<float,2,torch::RestrictPtrTraits>(),
                m_grid_extent,
                m_grid_translation_tensor.packed_accessor32<float,1,torch::RestrictPtrTraits>(),
