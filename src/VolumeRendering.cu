...
torch::Tensor VolumeRendering::integrate_with_weights(const RaySamplesPacked& ray_samples_packed, const torch::Tensor& rgb_samples, const torch::Tensor& weights_samples){
    ...
    int nr_rays=ray_samples_packed.ray_start_end_idx.size(0);
    int nr_dims=rgb_samples.size(1);
    // int nr_samples_total=ray_samples_packed.samples_z.size(0);
    ...
    torch::Tensor pred_rgb_per_ray=torch::zeros({ nr_rays,nr_dims },  torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)  );


    switch (nr_dims)
    {
    case 1:
        VolumeRenderingGPU::integrate_with_weights_gpu<1><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 3:
        VolumeRenderingGPU::integrate_with_weights_gpu<3><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 16:
        VolumeRenderingGPU::integrate_with_weights_gpu<16><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 32:
        VolumeRenderingGPU::integrate_with_weights_gpu<32><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 48:
        VolumeRenderingGPU::integrate_with_weights_gpu<48><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 64:
        VolumeRenderingGPU::integrate_with_weights_gpu<64><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
...
std::tuple<torch::Tensor, torch::Tensor> VolumeRendering::integrate_with_weights_backward(const torch::Tensor& grad_pred_rgb, const RaySamplesPacked& ray_samples_packed, const torch::Tensor& rgb_samples, const torch::Tensor& weights_samples, const torch::Tensor& pred_rgb){
    ...
    int nr_dims = grad_pred_rgb.size(1);

    //fill the samples
    const dim3 blocks = { (unsigned int)div_round_up(nr_rays, BLOCK_SIZE), 1, 1 }; 


    torch::Tensor grad_rgb_samples=torch::zeros({ nr_samples_total,nr_dims },  torch::dtype(torch::kFloat32).device(torch::kCUDA, 0)  );
    ...
    {
    case 1:
        VolumeRenderingGPU::integrate_with_weights_backward_gpu<1><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 3:
        VolumeRenderingGPU::integrate_with_weights_backward_gpu<3><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 16:
        VolumeRenderingGPU::integrate_with_weights_backward_gpu<16><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 32:
        VolumeRenderingGPU::integrate_with_weights_backward_gpu<32><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 48:
        VolumeRenderingGPU::integrate_with_weights_backward_gpu<48><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
    ...
    case 64:
        VolumeRenderingGPU::integrate_with_weights_backward_gpu<64><<<blocks, BLOCK_SIZE>>>(
            nr_rays,
            ray_samples_packed.max_nr_samples, //useful for checking if the ray has samples higher the the max_nr_samples in which case we don't integrate
            ray_samples_packed.ray_start_end_idx.packed_accessor32<int,2,torch::RestrictPtrTraits>(),
            ray_samples_packed.rays_have_equal_nr_of_samples,
