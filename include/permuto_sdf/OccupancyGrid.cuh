...
class OccupancyGrid{
public:
    OccupancyGrid(const int nr_voxels_per_dim, const float grid_extent, const torch::Tensor &grid_translation);
    ~OccupancyGrid();
    ...
    void update_with_sdf_positions(const torch::Tensor &points, const torch::Tensor &sdf, const float inv_s, const float occupancy_thresh);
