...
__device__ void get_start_end_ray_indices(int& nr_samples_per_ray, int& out_idx_start, int& out_idx_end, const int idx_ray, const bool rays_have_equal_nr_of_samples, const int fixed_nr_of_samples_per_ray, const torch::PackedTensorAccessor32<int,2,torch::RestrictPtrTraits>& ray_start_end_idx  ){ 
...
integrate_with_weights_gpu(
    ...
    // if (false){ //this batch of samples would have ended up outside of the maximum allocation of samples but we didn't actually write any samples
        ...
        for (int i=0; i<nr_samples; i++) {
            ...
            #pragma unroll
            for (int d=0; d<D; ++d) {
                feat_ray[d] += weight * rgb_samples[idx_start+i][d];
            }
        }
        //finish
        #pragma unroll
        for (int d=0; d<D; ++d)
            pred_rgb_per_ray[idx][d] = feat_ray[d];
    }
}

template<>
__global__ void 
integrate_with_weights_gpu<1>(
    const int nr_rays,
    const int max_nr_samples,
    const torch::PackedTensorAccessor32<int,2,torch::RestrictPtrTraits> ray_start_end_idx,
    const bool rays_have_equal_nr_of_samples,
    ...
    // if (false){ //this batch of samples would have ended up outside of the maximum allocation of samples but we didn't actually write any samples
        ...
        for (int i=0; i<nr_samples; i++) {
            //integrate
            float depth = rgb_samples[idx_start+i][0];
            float weight=weights_samples[idx_start+i][0];
            depth_ray += weight * depth;
        }
        //finish
        pred_rgb_per_ray[idx][0]=depth_ray;
    }
}
...
integrate_with_weights_backward_gpu(
    ...
    // if (false){ //this batch of samples would have ended up outside of the maximum allocation of samples but we didn't actually write any samples
        ...
        float grad_pred_feat_cur_ray[D];
        #pragma unroll
        for (int d = 0; d < D; ++d)
            grad_pred_feat_cur_ray[d] = grad_pred_rgb[idx][d];
...
template<>
__global__ void 
integrate_with_weights_backward_gpu<1>(
    const int nr_rays,
    const int max_nr_samples,
    const torch::PackedTensorAccessor32<int,2,torch::RestrictPtrTraits> ray_start_end_idx,
    const bool rays_have_equal_nr_of_samples,
    ...
    // if (false){ //this batch of samples would have ended up outside of the maximum allocation of samples but we didn't actually write any samples
        ...
        float grad_pred_depth_cur_ray = grad_pred_rgb[idx][0];

        for (int i=0; i<nr_samples; i++) {
            float depth_cur_sample = rgb_samples[idx_start+i][0];
            float weight_cur_sample=weights_samples[idx_start+i][0];
            grad_rgb_samples[idx_start+i][0]=grad_pred_depth_cur_ray * weight_cur_sample;
            grad_weights_samples[idx_start+i][0]=grad_pred_depth_cur_ray * depth_cur_sample;
        }
    }
}
...
template<>
__global__ void 
integrate_with_weights_backward_gpu<3>(
    const int nr_rays,
    const int max_nr_samples,
    const torch::PackedTensorAccessor32<int,2,torch::RestrictPtrTraits> ray_start_end_idx,
    const bool rays_have_equal_nr_of_samples,
